...
export class WalletConnectConnector implements IWalletConnector {
  ...
  private constructor(
    cfg: { projectId: string; chains?: Chain[]; defaultChainId?: number },
  ...
  onSessionConnected(_session: SessionTypes.Struct) {
    const firstConfiguredChain = this.chains[0];

    let accounts: Address[] = [];
    let chainId = firstConfiguredChain.id;

    // 从session中查找我们配置的第一个chain的地址
    if (_session.namespaces.eip155) {
      const eip155Namespace = _session.namespaces.eip155;
      // 查找我们配置的第一个chain上的账号
  ...
  getPublicClient(chainId?: number) {
    const chain = this.getChain(
      chainId ?? this.state.chainId ?? this.defaultChainId
    );
