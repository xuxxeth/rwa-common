 ...
 * Context
 */
export const WalletConnectCtx = createContext<IContext>({} as IContext);
...
export function WalletConnectProvider({
  ...
  const [chains, setChains] = useState<string[]>(_config.chains.map(chain => 'eip155:' + chain.id));
  ...
  const reset = () => {
    ...
    setAccounts([]);
    setChains([]);
    setRelayerRegion(_config.relayUrl!);
  };
...
export function useWalletConnectCtx() {
  const context = useContext(WalletConnectCtx);
  if (context === undefined) {
    throw new Error(
      "useWalletConnectCtx must be used within a WalletConnectCtxProvider"
    );
  }
  return context;
}
